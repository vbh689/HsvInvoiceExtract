...
**Timing:** the request is synchronous — no polling, no webhook. On a malformed or unreliable first response the service retries once against the same model before giving up, so budget for that in your HTTP client timeout (`LLM_TIMEOUT_S` per attempt, default 25s — allow at least 2x that).
...
- **`usage`** — one attempt or two (`attempts: 2` means the first response was malformed/unusable and the service retried once against the same model), latency, tokens, cost, and which prompt/schema version produced this response. `cached: true` means an identical document (by content hash) was seen before and the cached reconciled result was returned instead of calling a model again — `tokens_*` and `cost_usd`/`cost_vnd` are `0` on a cache hit. Re-uploading the exact same file is cheap and fast. `cost_vnd` is `cost_usd` converted at a fixed rate (`settings.usd_to_vnd_rate`, currently 26500) — not a live FX lookup, so don't treat it as accurate to the day. Set `EXPOSE_USAGE_IN_RESPONSE=false` to omit this field entirely from the response (it's `null` in that case) — it's still recorded in the audit log/dashboard either way.
...
- **Idempotency / retries:** identical file bytes hash to the same cache entry, so retrying a failed request with the same file is safe and cheap (`usage.cached: true` on the retry).
